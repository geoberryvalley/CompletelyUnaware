...
def openTodayAlgo():
    ...
    for symbol, price in symbol_price_dict.items():
        ...
        db_functions.open_new_position(symbol, size, price)
...
def closeAllPositions():
    ...
    oldPositions = db_functions.get_open_positions()
    if not oldPositions:
        ...
        assert type(response) == dict[str, dict[str, float]], "Response from Alpaca API is not in the expected format."
